...
def upload_media_dialog(p_id):
    ...
    if upload_type == "Image":
        ...
        if uploaded_file:
            ...
            if not orig_fps: orig_fps = 30.0
            
            st.markdown(f"**Video Info:** {orig_fps:.1f} FPS | {orig_total_frames} total frames")
            
            target_fps = st.slider("Extraction Rate (Frames Per Second)", min_value=0.5, max_value=float(orig_fps), value=1.0, step=0.5, help="Higher FPS means more images to process.")
            
            duration = orig_total_frames / orig_fps if orig_fps > 0 else 0
            expected_frames = int(duration * target_fps)
            
            st.info(f"📸 Approximately **{expected_frames} frames** will be extracted and uploaded.")
                ...
                with st.spinner("Extracting frames and uploading to Supabase..."):
                    ...
                    while True:
                        ...
                        if count % frame_interval == 0:
                            ...
                            if ret: frames.append(buf.tobytes())
                        count += 1
                    cap.release()
                    try:
                        os.remove(tmp_path)
    ...
    if media_df.empty:
        ...
        completed_img_rows = {
            ...
            if r['status'] == 'completed'
               and r.get('file_path', '')
               and any(r['file_path'].lower().split('?')[0].endswith(e)
        ...
        for group_key, rows in media_groups.items():
            if group_key.startswith("single_"):
                ...
                with st.expander(label, expanded=False):
                    ...
                    if is_image:
                        ...
                        if show_detections and row['status'] == 'completed':
                            if pd.notna(row.get('storage_path')) and row['storage_path']:
                                display_path = row['storage_path']
                            else:
                                display_path = f"projects/{proj_id}/detections/{row['id']}/annotated.jpg"
                        st.image(signed_image_url(display_path, cache_bust=st.session_state.img_cache_bust), width="stretch")
                    else:
                ...
                with st.expander(label, expanded=False):
                    ...
                    with hdr_left:
                        if st.session_state.user and st.session_state.user.id == user_id:
                            if st.button("🗑️ Delete Entire Batch",
                                         key=f"del_batch_{group_key}"):
                    ...
                    with hdr_right:
                        if batch_completed_ids:
                            def _on_batch_select(ids=batch_completed_ids, key=f"gal_batch_sel_{group_key}"):
                    ...
                    for idx, row in enumerate(rows):
                        with cols[idx % 3]:
                            ...
                            if row['status'] == 'completed' and is_image:
                                chk = st.checkbox(
                                    clean, value=row['id'] in st.session_state.gallery_selected,
                                    key=f"gal_chk_{row['id']}"
                                )
                                if chk:
                                    st.session_state.gallery_selected.add(row['id'])
        ...
        # ── Download bar ───────────────────────────────────────────────────────
        selected_in_proj = [
            mid for mid in st.session_state.gallery_selected
            if mid in all_completed_ids
        ]
        if selected_in_proj:
            st.divider()
            ...
            with bar_right:
                if st.button("📦 Prepare & Download ZIP", type="primary",
                             width="stretch", key="gal_dl_zip"):
                    id_to_name = {
                    ...
                    with zipfile.ZipFile(zip_buf, mode="w",
                        ...
                        for idx, mid in enumerate(selected_in_proj):
                            ...
                            storage_path = f"projects/{proj_id}/detections/{mid}/annotated.jpg"
                            ...
                            try:
                                ...
                                arc_name = (base if base.lower().endswith(
                                    ('.jpg', '.jpeg', '.png')) else base + '.jpg')
                ...
                if is_owner:
                    if st.button("🗑️ Delete Selected Detections", type="secondary", width="stretch", key="gal_del_det"):
                        with st.spinner("Deleting selected detections..."):
                            db.delete_detections_for_media(selected_in_proj, project_id=proj_id)
                            st.session_state.gallery_selected -= set(selected_in_proj)
                        st.success(f"✅ Cleared detections for {len(selected_in_proj)} image(s).")
                        st.session_state.img_cache_bust = int(time.time())
                        import time
                        time.sleep(1)
                        st.rerun()
    ...
    if not st.session_state.user:
        ...
        if candidate_media.empty:
            if is_redo:
                ...
                st.info("✅ All media has already been processed. Check the 🤖 AI Detections tab, or switch to **Re-run** mode to redo them.")
        else:
            ...
            if is_redo:
                st.caption("⚠️ Existing detections and annotated images for the selected media will be permanently deleted before re-processing.")
            ...
            if st.button(btn_label, type="primary", width="stretch"):
                if selected_keys:
                    ...
                    use_yolov11   = v11_available  and model_choice == "YOLOv11 - Road Damage (Detection)"
                    use_yolov8seg = v8seg_available and model_choice == "YOLOv8 - Road Damage (Segmentation)"
                    use_real_model = use_yolov11 or use_yolov8seg
                    chosen_model_type = (
                        ai_processor.MODEL_YOLOV8SEG if use_yolov8seg
                    ...
                    if is_redo:
                        with st.spinner("Clearing previous detections..."):
                            db.delete_detections_for_media(selected_ids, project_id=proj_id)

                    st.info(f"🚀 Initializing AI Run: {model_choice} (Type: {chosen_model_type})")
...
with tab_detections:
    if det_df.empty:
        ...
        filtered_det_df = det_df[det_df['damage_type'].isin(selected_types)] if selected_types else det_df
        ...
        group_display = {
            gk: (rows[0]['filename'] if gk.startswith("single_") else f"{gk} ({len(rows)} images)")
            for gk, rows in completed_groups.items()
        }
        if is_owner:
            with st.expander("🗑️ Delete Detections", expanded=False):
                st.caption("Deleting detections removes results from the database and annotated images from Storage, then resets the media back to *pending* so it can be re-processed.")
                ...
                if completed_groups:
                    ...
                    with col_del1:
                        if st.button("🗑️ Delete Selected Detections", type="secondary", width="stretch", disabled=not del_keys):
                            ids_to_clear = []
                            for gk in del_keys:
                                ids_to_clear.extend([r['id'] for r in completed_groups[gk]])
                            with st.spinner("Deleting detections..."):
                                db.delete_detections_for_media(ids_to_clear, project_id=proj_id)
                            st.success(f"✅ Cleared detections for {len(del_keys)} upload(s).")
                            time.sleep(1)
                            st.rerun()
                    with col_del2:
                        all_completed_ids = [r['id'] for rows in completed_groups.values() for r in rows]
                        if st.button("💣 Delete ALL Detections", type="primary", width="stretch"):
                            with st.spinner("Deleting all detections for this project..."):
                                db.delete_detections_for_media(all_completed_ids, project_id=proj_id)
                            st.success("✅ All detections cleared. Media reset to pending.")
                            time.sleep(1)
                            st.rerun()
                else:
        ...
        for group_key, rows in media_groups.items():
            ...
            group_det = filtered_det_df[filtered_det_df['media_id'].isin(group_media_ids)]
            if group_det.empty:
                continue
            any_shown = True
